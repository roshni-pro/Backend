//Created by Pravesh 08-08-2019 
// SB1-T1157 Order Data Download Only for Kisan Kirana

'use strict';
app.controller('OrderDataKKController', ['$scope', 'OrderMasterService', 'OrderDetailsService', '$http', 'ngAuthSettings', '$filter', "ngTableParams", '$modal', "DeliveryService", "WarehouseService", 'CityService',
    function ($scope, OrderMasterService, OrderDetailsService, $http, ngAuthSettings, $filter, ngTableParams, $modal, DeliveryService, WarehouseService, CityService) {
        var UserRole = JSON.parse(localStorage.getItem('RolePerson'));
       {
            ...
            $scope.getSearchdata = function (startdate, enddate, WarehouseId) { // This would fetch the data on page change.  //In practice this should be in a factory.
                ...
                var url = serviceBase + "api/OrderDispatchedMaster/OrderDataDownloadKK" + "?datefrom=" + startdate + "&dateto=" + enddate +"&WarehouseId=" + $scope.WarehouseId;
            ...
            $scope.exportData = function (data) {
                ...
                alasql('SELECT CustomerId,SKCode,ShopName,WarehouseName,Address,NOOfOrder,TotalAmount,ExcutiveName INTO XLSX("OrderDataKisanKirana.xlsx",{headers:true}) FROM ?', [$scope.NewExportData]);
...
//*************************************** New fuction for Advance Search*************************************************//
            ...
            $scope.getSearchADVdata = function (Demanddata) { // This would fetch the data on page change.  //In practice this should be in a factory.
                ...
                var url = serviceBase + "api/OrderDispatchedMaster/OrderDataAdvanceKK" + "?Year=" + Demanddata.Year + "&Cityid=" + Demanddata.Cityid + "&WarehouseId=" + Demanddata.WarehouseId;
                $http.get(url).success(function (response) {
            ...
            if ($scope.newOrderByDate.length <= 0) {
                ...
                alasql('SELECT skcode,shopname,warehouseid,warehousename,executiveid,executivename,Jan,Feb,Mar,Apr,May,June,July,Aug,Sep,Oct,Nov,Dec,totalorderCount,TotalAmount INTO XLSX("YearKisanKirana.xlsx",{headers:true}) FROM ?', [$scope.NewSearchExportData]);
            };   
        }
    }]);
