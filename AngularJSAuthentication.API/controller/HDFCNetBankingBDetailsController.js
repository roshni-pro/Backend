'use strict';
app.controller('HDFCNetBankingBDetailsController', ['$scope', "$filter", "$http", "ngTableParams", '$modal', 'localStorageService', '$routeParams', function ($scope, $filter, $http, ngTableParams, $modal, localStorageService, $routeParams) {
    ...
    $scope.Getdata = function () {
        ...
        var url = serviceBase + "api/OnlineTransaction/GethdfcNetDetails?UploadId=" + $scope.upload;
    ...
    $scope.Comment = function (item) {
        ...
        modalInstance = $modal.open(
            {
                templateUrl: "HdfcNetBankingcomment.html",
                controller: "ModalInstancehdfcNetComment", resolve: { city: function () { return item } }
    ...
    $scope.selecthdfcNet = function (data) {
        
        $scope.paymentdetail = [];
        var url = serviceBase + "api/OnlineTransaction/OnchangeHDFCNet?UploadId=" + $scope.upload + "&status=" + data.status;
        $http.get(url)
...
app.controller("ModalInstancehdfcNetComment", ["$scope", '$http', 'ngAuthSettings', "$modalInstance", "city", 'FileUploader', function ($scope, $http, ngAuthSettings, $modalInstance, city, FileUploader) {
    console.log("city");
