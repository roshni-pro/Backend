...
(function () {
    ...
    cityController.$inject = ['$scope', "CityService", 'StateService', "$filter", "$http", "ngTableParams", '$modal'];

    function cityController($scope, CityService, StateService, $filter, $http, ngTableParams, $modal) {
...
(function () {
    ...
    angular
        .module('app')
        .controller('ModalInstanceCtrlCity', ModalInstanceCtrlCity);

    ModalInstanceCtrlCity.$inject = ["$scope", '$http', 'ngAuthSettings', "CityService", "StateService", "$modalInstance", "city", 'FileUploader'];

    function ModalInstanceCtrlCity($scope, $http, ngAuthSettings, CityService, StateService, $modalInstance, city, FileUploader) {
        console.log("city");
        ...
        $scope.AddCity = function (data) {
            //    
            if ($scope.CityData.aliasName == null && $scope.CityData.aliasName == undefined) {
                alert('enter the alias name');
                return;
            }
            console.log("AddCity");
            ...
            $http.post(url, dataToPost)
                .success(function (data) {
                    ...
                    $scope.AddTrack("Add(City)", "NewCity:", dataToPost.CityName);
        ...
        $scope.PutCity = function (data) {
            ...
            var url = serviceBase + "api/City";
            var dataToPost = {
                Cityid: $scope.CityData.Cityid,
                CityName: $scope.CityData.CityName,
                aliasName: $scope.CityData.aliasName,
                CreatedDate: $scope.CityData.CreatedDate,
                UpdatedDate: $scope.CityData.UpdatedDate,
                CreatedBy: $scope.CityData.CreatedBy,
                UpdateBy: $scope.CityData.UpdateBy,
                Stateid: $scope.CityData.Stateid,
                Code: $scope.CityData.Code,
                active: $scope.CityData.active, //add for retailer
                IsSupplier: $scope.CityData.IsSupplier // add for Supplier
            };
            ...
            $http.put(url, dataToPost)
                .success(function (data) {
                    console.log("PutCity");
                    $scope.AddTrack("Edit(City)", "CityName:", dataToPost.CityName);
                    console.log(data);
                    if (data.id == 0) {
                        $scope.gotErrors = true;
                        if (data[0].exception == "Already") {
...
(function () {
    ...
    angular
        .module('app')
        .controller('ModalInstanceCtrldeleteCity', ModalInstanceCtrldeleteCity);

    ModalInstanceCtrldeleteCity.$inject = ["$scope", '$http', "$modalInstance", "CityService", 'ngAuthSettings', "city"];

    function ModalInstanceCtrldeleteCity($scope, $http, $modalInstance, CityService, ngAuthSettings, myData) {
