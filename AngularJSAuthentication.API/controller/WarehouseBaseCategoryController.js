...
(function () {
    ...
    angular
        .module('app')
        .controller('WarehouseBaseCategoryController', WarehouseBaseCategoryController);

    WarehouseBaseCategoryController.$inject = ['$scope', 'CityService', 'StateService', "WarehouseService", "CategoryService", "WarehouseCategoryService", "$filter", "$http", "ngTableParams", '$modal'];

    function WarehouseBaseCategoryController($scope, CityService, StateService, WarehouseService, CategoryService, WarehouseCategoryService, $filter, $http, ngTableParams, $modal) {
        ...
        WarehouseService.getwarehousedistinctstates().then(function (results) {
            ...
            $scope.WarhouseStatesBase = results.data;
        }, function (error) {
        });
        ...
        $scope.GetWarhouseCategory = function (WareHouse) {
            ...
            CategoryService.getWarhouseBaseCategory(WareHouse).then(function (results) {
                console.log(results.data);
        ...
        $scope.cancel = function () { $modalInstance.dismiss('canceled'); };
            $scope.checkAll = function () {
                ...
                angular.forEach($scope.Wcategorys, function (category) {
                    $scope.checkboxes.items[category.BaseCategoryId] = $scope.selectedAll;
                });
        ...
        $scope.$watch('checkboxes.checked', function (value) {
            ...
            angular.forEach($scope.Wcategorys, function (category) {
                if (angular.isDefined(category.BaseCategoryId)) {
                    $scope.checkboxes.items[category.BaseCategoryId] = value;
                }
            });
        });
        ...
        $scope.$watch('checkboxes.items', function (values) {
            ...
            angular.forEach($scope.Wcategorys, function (category) {
                checked += ($scope.checkboxes.items[category.BaseCategoryId]) || 0;
                unchecked += (!$scope.checkboxes.items[category.BaseCategoryId]) || 0;
            });
        ...
        $scope.AddWhBaseCategory = function () {
            ...
            for (var j = 0; j < $scope.CkdId.length; j++) {
                _.each($scope.Wcategorys, function (o2) {
                    if (o2.BaseCategoryId == $scope.CkdId[j]) {
                        $scope.plist.push(o2);
                    }
                })
            ...
            var url = serviceBase + "api/WarehouseCategory/BaseCategory";
        ...
        $scope.getData = function (WarehouseId) {
            WarehouseCategoryService.getwhBasecategoryswid(WarehouseId).then(function (results) {
        ...
        $scope.Activecate = function (item) {
            ...
            modalInstance = $modal.open(
                {
                    templateUrl: "activebasewarehousecmodal.html",
                    controller: "WarehousebaseCategoryeditctrl", resolve: { whcategory: function () { return item } }
                });
...
(function () {
    ...
    angular
        .module('app')
        .controller('WarehousebaseCategoryeditctrl', WarehousebaseCategoryeditctrl);

    WarehousebaseCategoryeditctrl.$inject = ["$scope", '$http', 'ngAuthSettings', "$modalInstance", "whcategory", 'FileUploader'];

    function WarehousebaseCategoryeditctrl($scope, $http, ngAuthSettings, $modalInstance, whcategory, FileUploader) {
        ...
        $scope.ok = function () { $modalInstance.close(); };
            ...
            $scope.putwarehouseBasecat = function (data) {
                ...
                var url = serviceBase + "api/WarehouseCategory/WHBaseCatAct";
                var dataToPost = {
