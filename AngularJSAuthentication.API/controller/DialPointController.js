...
(function () {
    ...
    angular
        .module('app')
        .controller('DialPointController', DialPointController);

    DialPointController.$inject = ['$scope', "$http", "localStorageService", "peoplesService", "Service", "$modal", "ngTableParams", "$filter"];

        function DialPointController($scope, $http, localStorageService, peoplesService, Service, $modal, ngTableParams, $filter) {
            ...
            // Getting Brand of Warehouse
            $scope.getDialPoint = function () {
                $scope.DialPointData = [];
                var url = serviceBase + "api/DialPoint";
            ...
            $scope.SearchDialPoint = function () {
                ...
                var url = serviceBase + "api/DialPoint/Search?start=" + start + "&end=" + end + "&skcode=" + $scope.skcode;
                $http.get(url).success(function (results) {
                    $scope.DialPointData = results;
            ...
            $scope.exportData = function () {
                alasql('SELECT CustomerName,ShopName,Skcode,BillingAddress,point,OrderAmount,OrderId,UsedUnused,expired, CreatedDate INTO XLSX("DialPoint.xlsx",{headers:true}) FROM ?', [$scope.DialPointData]);
            };
    }
})();
