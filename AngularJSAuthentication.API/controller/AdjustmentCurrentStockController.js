...
(function () {
   // 'use strict';
    ...
    angular
        .module('app')
        .controller('AdjustmentCurrentStockController', AdjustmentCurrentStockController);

    AdjustmentCurrentStockController.$inject = ['CurrentStockService', 'WarehouseService', '$modal', '$scope', "$filter", "$http", "ngTableParams", "$interval"];

    function AdjustmentCurrentStockController(CurrentStockService, WarehouseService, $modal, $scope, $filter, $http, ngTableParams, $interval) {
        ...
        function handleFileUpload(files, obj) {
            ...
            $scope.getcurrentstock = function (WarehouseId) {
                ...
                CurrentStockService.getAdjstockWarehousebased(WarehouseId).then(function (results) {
            ...
            $scope.exportData1 = function () {
                ...
                alasql('SELECT ItemNumber,ItemMultiMRPId,StockId,ABCCategory,itemname,CurrentInventory,PlannedStock,WarehouseName INTO XLSX("Currentstock.xlsx",{headers:true}) FROM ?', [$scope.stores]);
            };
            ...
            $scope.edit = function (item) {
                ...
                modalInstance = $modal.open(
                    {
                        templateUrl: "AdjmyInventoryModalPut.html",
                        controller: "AdjModalInstanceCtrlInventoryedit", resolve: { inventory: function () { return item } }
                    });
            ...
            $scope.HistoryexportData = function () {
                ...
                var url = serviceBase + "api/CurrentStock/Export" + "?StockId=" + $scope.StockId + "&WarehouseId=" + $scope.WarehouseId;
                $http.get(url).success(function (response) {

                    $scope.exportDataRecord = response;


                    alasql('SELECT ItemNumber,itemname,ManualInventoryIn,InventoryIn,InventoryOut,DamageInventoryOut,PurchaseInventoryOut,OrderCancelInventoryIn,TotalInventory,OdOrPoId,WarehouseName,UserName,CreationDate INTO XLSX("CurrentItemstockHistory.xlsx",{headers:true}) FROM ?', [$scope.exportDataRecord]);
...
//    angular
//        .module('app')
//        .controller('AdjModalInstanceCtrlInventoryedit', AdjModalInstanceCtrlInventoryedit);

//    AdjModalInstanceCtrlInventoryedit.$inject = ["$scope", '$http', 'ngAuthSettings', "$modalInstance", "inventory"];

//    function AdjModalInstanceCtrlInventoryedit($scope, $http, ngAuthSettings, $modalInstance, inventory) {
...
//            if ($scope.inventoryData.CurrentInventory > 0) {
...
//                alert('please enter  for the positive Qty');
//            }
//        }
//    }
//})();
...
(function () {
    ...
    angular
        .module('app')
        .controller('AdjEmptyCurrentController', AdjEmptyCurrentController);

    AdjEmptyCurrentController.$inject = ['CurrentStockService', 'WarehouseService', '$modal', '$scope', "$filter", "$http", "ngTableParams"];

    function AdjEmptyCurrentController(CurrentStockService, WarehouseService, $modal, $scope, $filter, $http, ngTableParams) {
        ...
        $scope.oldStock = function (data) {
            ...
            $scope.AddTrack("View(History)", "ItemName:", "");
            $scope.oldStocks($scope.pageno);
        }
        $scope.oldStocks = function (pageno) {
