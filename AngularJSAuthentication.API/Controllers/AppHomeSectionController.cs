...
{
    [RoutePrefix("api/AppHomeSection")]
    public class AppHomeSectionController : ApiController
    {
        ...
        public dynamic GetSections(string appType, int wId)
        {
            using (var context = new AuthContext())
            {
                ...
                if (!string.IsNullOrEmpty(appType) && wId > 0)
                {
                    ...
                    var sectionsResult = context.AppHomeSectionsDB.Where(x => x.WarehouseID == wId && x.AppType == appType && x.Deleted == false).Include(o => o.AppItemsList)
        ...
        public AppHomeSections AddSections(AppHomeSections appsection)
        {
            using (var context = new AuthContext())
            {
                ...
                if (appsection.AppItemsList != null)
                {
                    ...
                    var existingAppItems = ar.AppItemsList; // context.AppHomeSectionItemsDB.Where(x => x.apphomesections.SectionID == ar.SectionID).ToList();
                ...
                else
                {
                    ...
                    ar.HeaderTextSize = appsection.HeaderTextSize;
                    ...
                    if (appsection.AppItemsList != null)
                    {
                        foreach (var o in appsection.AppItemsList)
                        {
                                ...
                                else if (appsection.SectionSubType == "Slider")
                                {
                                    ...
                                    else if (o.RedirectionType == "Category")
                                    {
                                        ...
                                        foreach (var Catdata in itemBaseCats)
                                        {
                                            ...
                                            o.Active = true;
                                            context.AppHomeSectionItemsDB.Add(o);
                                            //Dated 25/05/2022 by Manoj Bhardwaj
                                            context.SaveChanges();
                                        }
                                    }
                                    else if (o.RedirectionType == "Base Category")
                            ...
                            else
                            {
                                ...
                                appItem.ImageLevel = o.ImageLevel;
                                appItem.HasOffer = o.HasOffer;
                                if (appItem.HasOffer || appItem.IsFlashDeal)
                                {
                                    appItem.OfferStartTime = o.OfferStartTime;
                                    appItem.OfferEndTime = o.OfferEndTime;
                                }
        ...
        [Route("CloneAppHome")]
        [AcceptVerbs("POST")]
        public HttpResponseMessage CloneAppHome(string appTypefrom, int wIdfrom, string appType, int wId)
        {
            using (var context = new AuthContext())
            {
        ...
        [Route("DeleteAppHome")]
        [AcceptVerbs("POST")]
        public HttpResponseMessage DeleteCompleteAppHome(List<AppHomeSections> apphome)
        {
            using (var context = new AuthContext())
            {
        ...
        public bool DeleteItem(int SectionID, int SectionItemID)
        {
            using (var context = new AuthContext())
            {
                ...
                if (ara != null)
                {
                    ...
                    if (context.Commit() > 0)
                    {
                        if (ara.RedirectionType == "Flash Deal")
                        {
                            ManulaExpireFlasDeal(SectionItemID);
                        }
                    }
                    return true;
                }
        ...
        public List<AppHomeSections> PublishAppHome(List<AppHomeSections> apphome)
        {
            using (var context = new AuthContext())
            {
                ...
                if (publishData != null && result)
                {
                    ...
                    var apphomeresult = _cacheProvider.GetOrSet(Caching.CacheKeyHelper.APPHomeCacheKey(apptype, apphome.FirstOrDefault().WarehouseID.ToString()), () => publishData);
#endif
        ...
        public IEnumerable<AppHomeSections> GetPublishedSection(string appType, int wId, string lang, int CustomerId)
        {
            ...
            using (var context = new AuthContext())
            {
                ...
                else
                {
                    ...
                    //sections.Where(x => x.SectionSubType == "Category").ToList().ForEach(x =>
                    //{
                    //    if (x.AppItemsList != null && x.AppItemsList.Any())
                    //    {
                    //        x.AppItemsList = x.AppItemsList.Where(y => !excludecategory.Contains(y.CategoryId)).ToList();
                    //    }
                    //});
...
#region Today Flash Deal Dashboard (21-11-2019)
        ...
        [Route("AppHomeRequiredData")]
        [HttpGet]
        public AppHomeRequiredData GetAppHomeRequiredData()
        {
            AppHomeRequiredData appHomeRequiredData = new AppHomeRequiredData();
            using (var context = new AuthContext())
            {
                appHomeRequiredData.AppHomeBaseCategoryDc = context.BaseCategoryDb.Where(x => x.Deleted == false && x.IsActive == true)
                    .Select(x => new AppHomeBaseCategoryDc
                    {
                ...
                appHomeRequiredData.AppHomeCategoryMinDc = context.Categorys.Where(x => x.Deleted == false && x.IsActive == true)
                    .Select(x => new AppHomeCategoryMinDc
                    {
                        BaseCategoryId = x.BaseCategoryId,
                ...
                appHomeRequiredData.SubsubCategoryDTOM = context.Database.SqlQuery<SubsubCategoryDTOM>(query).ToList();

                appHomeRequiredData.SubCategorysDTO = context.SubCategorys.Where(x => x.IsActive && !x.Deleted).Select(x => new SubCategoryController.SubCategorysDTO
                {
        ...
        public HttpResponseMessage GetWarehouseItemWithInactive(int WarehouseId)//get all Issuances which are active for the delivery boy
        {
            using (var db = new AuthContext())
            {
                if (WarehouseId > 0)
                {
                    var Item = db.itemMasters.Where(x => x.WarehouseId == WarehouseId && x.Deleted == false).Select(x => new
                    {
                        x.ItemId,
                        x.itemname,
                        x.ItemMultiMRPId,
    ...
    public class AppHomeRequiredData
    {
        public List<SubCategoryController.SubCategorysDTO> SubCategorysDTO { get; set; }
        public List<SubsubCategoryDTOM> SubsubCategoryDTOM { get; set; }
        public List<AppHomeCategoryMinDc> AppHomeCategoryMinDc { get; set; }
        public List<AppHomeBaseCategoryDc> AppHomeBaseCategoryDc { get; set; }
    }

    public class AppHomeCategoryMinDc
    {
        public int Categoryid { get; set; }
    ...
    public class AppHomeBaseCategoryDc
    {
        public int BaseCategoryId { get; set; }
